...
**Atendente (Diretor)**
No padrão Builder, esse papel é conhecido como *Diretor*. Ele é responsável por conduzir a ordem de montagem do combo, mas não conhece os detalhes de como cada item é efetivamente adicionado — essa responsabilidade é do Builder.

**ComboBuilder («interface»)**
É a interface que define os métodos de montagem do combo (adicionar lanche, batata, bebida e brinde). Por ser uma interface, o Atendente não precisa conhecer a implementação concreta que está sendo usada, apenas os métodos disponíveis. Essa é a ideia central do padrão: o Diretor depende de uma abstração, não de uma classe específica.

**Combo**
Representa o produto final, criado somente depois que todos os itens foram definidos pelo Builder.
...
Um ponto importante deste diagrama é que o Atendente (Diretor) nunca interage diretamente com o produto final durante a montagem — ele apenas emite instruções sequenciais ao Builder. Quem efetivamente monta e entrega o combo é o Builder.

Essa separação de responsabilidades é o que torna o padrão útil na prática: caso seja necessário criar uma variação do combo (por exemplo, um combo vegetariano ou um combo infantil), basta implementar um novo Builder que siga a mesma interface. A lógica do Atendente permanece inalterada, pois ele depende apenas da abstração `ComboBuilder`, e não de uma implementação específica.
